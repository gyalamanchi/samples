...
#WHERE o.total_amount > 1000 AND o.order_date >= '2025-01-01' AND o.order_date < '2026-01-01';
...
class TextToSQLFlow(Flow):
    ...
    async def execute_query(self):
        """Execute the generated SQL query against SQLite"""
        sql_query = self.state["sql_query"]
        
        execution_task = Task(
            description=f"Execute this SQL query against an SQLite database and return results:\n{sql_query}",
...
def setup_database():
    with engine.connect() as conn:
        # Drop existing tables (for demo purposes)
        ...
        conn.execute(text("""
            ...
            ('Jane Smith', 'jane@example.com', '2025-02-01')
        """))
        conn.execute(text("""
