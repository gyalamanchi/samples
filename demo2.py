...
def generate_sql(state: GraphState):
    ...
    return {"messages": state["messages"], "sql_query": sql_query, "db_config": db_config}
...
def validate_sql(state: GraphState):
    ...
    try:
        ...
        cur.close()
        conn.close()
        return {"messages": state["messages"], "sql_query": sql_query, "db_config": db_config, "is_valid": True}
    except Exception as e:
        return {"messages": state["messages"] + [HumanMessage(content=f"SQL Validation Error: {e}")], "sql_query": sql_query, "db_config": db_config, "is_valid": False}
...
def execute_sql(state: GraphState):
    ...
    try:
        ...
        cur.close()
        conn.close()
        return {"messages": state["messages"] + [HumanMessage(content=f"Results: {results}, columns: {column_names}")], "db_config": state["db_config"], "sql_query":sql_query}
    except Exception as e:
        return {"messages": state["messages"] + [HumanMessage(content=f"SQL Execution Error: {e}")], "db_config": state["db_config"], "sql_query":sql_query}
